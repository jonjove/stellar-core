    ...
    std::vector<std::string> const& assetCodes)
{
    ...
    std::string sqlJoin =
        ...
        "INNER JOIN (SELECT rowid, value FROM carray(?, ?, 'char*')) AS z ON x.rowid = z.rowid";
...
LedgerTxnRoot::Impl::bulkLoadTrustLines(std::vector<LedgerKey> const& keys)
{
    ...
    {
        entries = sociGenericBulkLoadTrustLines(
            mDatabase, accountIDs, issuers, assetCodes);
    }
    else
    {
        std::abort();
        //postgresSpecificBulkLoadTrustLines(mDatabase, accountIDs);
    }

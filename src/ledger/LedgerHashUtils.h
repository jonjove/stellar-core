  ...
  public:
    ...
    operator()(stellar::LedgerKey const& lk) const
    {
        ...
        case stellar::CLAIMABLE_BALANCE:
            ...
            break;
        case stellar::SPONSORSHIP:
            res = stellar::shortHash::computeHash(
                stellar::ByteSlice(&lk.sponsorship().sponsorshipID,
                                   sizeof(lk.sponsorship().sponsorshipID)));
            break;
        case stellar::PREAUTHORIZATION:
        {
            auto& pa = lk.preauthorization();
            res = stellar::shortHash::computeHash(
                stellar::ByteSlice(pa.accountID.ed25519().data(), 8));
            res ^= hash<stellar::Asset>()(pa.asset);
            break;
        }
        default:
            abort();
